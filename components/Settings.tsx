...
const SettingsTab: React.FC<SettingsProps> = ({ settings, onSave }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 overflow-y-auto p-4 pb-16">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
          ...
          <textarea
            ...
            onChange={handleChange}
            rows={5}
            className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all text-sm leading-relaxed"
          />
        </div>
      ...
      {/* Success Toast */}
      <div
        className={`fixed top-6 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-6 py-3 rounded-full shadow-xl transition-all duration-300 z-[60] flex items-center gap-2 ${
          showSuccess ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-8 pointer-events-none'
        }`}
      >
